                    ...
                    <ColumnDirective
                        ...
                        template={({ status }: UserData) => (
                            <article className={cn('status-column', status === 'user' ? 'bg-success-50': 'bg-light-300')}>
                                <div className={cn('size-1.5 rounded-full', status === 'user' ? 'bg-yellow-300' : 'bg-success-500')} />
                                    <h3 className={cn('font-inter text-xs font-medium', status === 'user' ?'bg-yellow-500': 'text-success-700')}>
                                        {status}
                                    </h3>
                            </article>
                        )}
